...
class DataLoader:
    ...
    def _assert_days(self, data):
        ...
        assert (
            days_acc == days_act
        ), "Days in accommodation mismatch with days in activity."
    ...
    def _assert_activities(self, data):
        ...
        with MariaDB_Adaptor() as session:
            activities = session.fetch_activities(list(activity_ids))
    ...
    def _flatten_visit_times(self):
        ...
        for day, day_activity in enumerate(self.data["activities"]):
            ...
            business_hours = {
                activity_id: (
                    activity_data["start_time_int"],
                    activity_data["end_time_int"],
                )
                for activity_id, activity_data in self.activities.items()
            ...
            for activity_id in day_visit_time.keys():
                intersection_time = find_time_intersections(
                    day_visit_time[activity_id], business_hours[activity_id]
                )

                visit_time.setdefault(activity_id, [])

                visit_time[activity_id].extend(
                    [
                        (time[0] + day * 96, time[1] + day * 96)
        ...
        for activity_id in list(visit_time.keys()):
            visit_time[activity_id] = [
                (
                    time_range[0],
                    time_range[1] - self.data["activities_stayTime"][activity_id],
                )
                for time_range in get_union(visit_time[activity_id])
                if time_range[1] - time_range[0]
                >= self.data["activities_stayTime"][activity_id]
            ]
                ...
                night_time.append(
                    [
                        (
                            ...
                            sleepTimes[day]["morning"]
                            + (day) * 96
                            - sleepTimes[day - 1]["sleepTime"],
                        )
                    ]
                )
    ...
    def get_durations(self, place_orders):
        ...
        for i, place in enumerate(place_orders):
            if place in self.data["activities_stayTime"]:
                ...
                result.append(
                    sleep_times[0]["morning"]
                    if i == 0
                    else sleep_times[i - 1]["sleepTime"]
                )
