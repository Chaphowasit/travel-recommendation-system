...
class RelationalDBModule(Module):
    ...
    def provide_async_session(self) -> (AsyncSession):
        ...
        return loop.run_until_complete(get_async_session())

    @provider
    def provide_place_repository(self, session: AsyncSession) -> (RelationalDBPlaceRepository):
        return RelationalDBPlaceRepository(session)

    @provider
    def provide_async_sqlalchemy_unit_of_work(
        self, session: AsyncSession, place_repo: RelationalDBPlaceRepository
    ) -> (AbstractUnitOfWork):
        return AsyncSQLAlchemyUnitOfWork(session, place_repo)
